...
### 教程
- 学习使用 API 交易: [API 使用指南](https://www.okx.com/docs-v5/trick_zh/#instrument-configuration)
- 学习使用Python交易现货: [Python 现货交易教程](https://www.okx.com/help/how-can-i-do-spot-trading-with-the-jupyter-notebook)
- 学习使用Python交易衍生品: [Python 衍生品交易教程](https://www.okx.com/help/how-can-i-do-derivatives-trading-with-the-jupyter-notebook)
...
点击跳转至官网创建V5APIKey的页面 [创建我的APIKey](https://www.okx.com/account/my-api)
...
- 未绑定IP且拥有交易或提币权限的APIKey，将在闲置14天之后自动删除。(模拟盘的 API key 不会被删除)
  ...
  - 用户可以在 [安全中心](https://www.okx.com/zh-hans/account/security) 中看到未绑定IP且拥有交易/提现权限的 APIKey 最近使用记录。
...
### 签名
生成`OK-ACCESS-SIGN`的请求头是对`timestamp + method + requestPath + body`字符串（+表示字符串连接），以及SecretKey，使用HMAC SHA256方法加密，通过Base-64编码输出而得到的。
...
- `body`是指请求主体的字符串，如果请求没有主体（通常为GET请求）则body可省略。如：`{"instId":"BTC-USDT","lever":"5","mgnMode":"isolated"}`
...
- [订单频道](https://www.okx.com/docs-v5/zh/#order-book-trading-trade-ws-order-channel)
- [账户频道](https://www.okx.com/docs-v5/zh/#trading-account-websocket-account-channel)
- [持仓频道](https://www.okx.com/docs-v5/zh/#trading-account-websocket-positions-channel)
- [账户余额和持仓频道](https://www.okx.com/docs-v5/zh/#trading-account-websocket-balance-and-position-channel)
- [爆仓风险预警推送频道](https://www.okx.com/docs-v5/zh/#trading-account-websocket-position-risk-warning)
- [账户greeks频道](https://www.okx.com/docs-v5/zh/#trading-account-websocket-account-greeks-channel)
...
`apiKey`:调用API的唯一标识。需要用户手动设置一个 `passphrase`:APIKey的密码 `timestamp`:Unix Epoch 时间戳，单位为秒，如 1704876947 `sign`:签名字符串，签名算法如下：先将`timestamp` 、 `method` 、`requestPath` 进行字符串拼接，再使用HMAC SHA256方法将拼接后的字符串和SecretKey加密，然后进行Base64编码
...
| > instType | String | 否 | 产品类型<br/>`SPOT`：币币<br/>`MARGIN`：币币杠杆<br/>`SWAP`：永续<br/>`FUTURES`：交割<br/>`OPTION`：期权<br/>`ANY`：全部 |
| > instFamily | String | 否 | 交易品种<br/>适用于交割/永续/期权 |
...
| > channel | String | 是 | 频道名 |
| > instType | String | 否 | 产品类型<br/>`SPOT`：币币<br/>`MARGIN`：币币杠杆<br/>`SWAP`：永续合约<br/>`FUTURES`：交割合约<br/>`OPTION`：期权<br/>`ANY`：全部 |
| > instFamily | String | 否 | 交易品种<br/>适用于交割/永续/期权 |
| > instId | String | 否 | 产品ID |
